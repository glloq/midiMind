...
std::vector<WiFiClient> WiFiHotspot::parseConnectedClients() const {
    ...
    // Dans une vraie implémentation, parser:
    // - /var/lib/misc/dnsmasq.leases pour DHCP
    // - hostapd_cli all_sta pour les stations WiFi
    
    // Simuler pour les tests
    if (running_) {
        WiFiClient testClient;
        testClient.macAddress = "aa:bb:cc:dd:ee:ff";
        testClient.ipAddress = "192.168.4.10";
        testClient.hostname = "iPhone";
        testClient.connectedSince = std::chrono::duration_cast<std::chrono::milliseconds>(
            std::chrono::system_clock::now().time_since_epoch()
        ).count();
        testClient.bytesReceived = 12345;
        testClient.bytesSent = 67890;
        testClient.signalStrength = -45;
