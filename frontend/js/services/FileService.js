...
//
// CORRECTIONS v3.0.2:
//   ✅ uploadFile() complet avec validation
//   ✅ deleteFile() complet avec confirmation
//   ✅ moveFile() implémenté
//   ✅ renameFile() implémenté
//   ✅ Gestion erreurs robuste
//   ✅ Progress callbacks pour upload
//
...
class FileService {
    constructor(backendService, eventBus, logger) {
        ...
        this.logger.info('FileService', '✓ Service initialized');
        
        // Écouter les événements backend
    ...
    _bindBackendEvents() {
        // Écouter la réception de la liste de fichiers
        this.eventBus.on('backend:event:files_list', (data) => {
            this._handleFilesList(data);
        });
        
        // Écouter les événements de fichier ajouté/supprimé
    ...
    async scanFiles(force = false) {
        ...
        try {
            this.logger.info('FileService', '🔍 Scanning files...');
            ...
            this.logger.info('FileService', 
                `✓ Scan complete: ${files.length} files in ${duration}ms`);
...
async uploadFile(file, onProgress = null) {
    ...
    try {
        ...
        // CORRECTION: Invalider cache et rafraîchir
        this.fileCache.clear();
        this.lastScanTimestamp = 0;
        
        this.logger.info('FileService', `âœ… Upload successful: ${file.name}`);
...
async uploadFile(file, onProgress = null, options = {}) {
    ...
    try {
        ...
        if (result.success !== false && result.data) {
            ...
            this.logger.info('FileService', `✓ File uploaded: ${fileData.name}`);
 ...
 * @returns {Promise<string>} Contenu base64 (sans préfixe data:)
 */
...
async deleteFile(fileId, skipConfirmation = false) {
    ...
    // Vérifier utilisation dans playlists (si model disponible)
    ...
    try {
        ...
        this.logger.info('FileService', `✓ File deleted: ${file.name}`);
    ...
    async moveFile(fileId, newPath) {
        ...
        this.logger.info('FileService', `Moving: ${file.name} → ${newPath}`);
        ...
        try {
            ...
            this.logger.info('FileService', `✓ Moved: ${file.name}`);
...
async renameFile(fileId, newName) {
    ...
    _isValidFileName(name) {
        ...
        // Interdire ".." pour éviter path traversal
    ...
    _handleFileAdded(data) {
        if (data.file) {
            ...
            this.logger.info('FileService', `➕ File added: ${file.name}`);
        }
    }
    ...
    _handleFileRemoved(data) {
        if (data.fileId || data.path) {
            ...
            if (file) {
                ...
                this.logger.info('FileService', `➖ File removed: ${file.name}`);
            }
        }
    }
}
